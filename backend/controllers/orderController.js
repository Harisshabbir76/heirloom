...
function isAdminRequest(req) {
  ...
  const adminEmail = String(
    process.env.DASHBOARD_ACCESS_ADMIN_EMAIL || ''
  )
    .trim()
    .toLowerCase();

  if (!token || !adminEmail || !process.env.JWT_SECRET) {
    return false;
  }
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: 'Order items and email are required',
      });
    }
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    if (
      !['new', 'in-process', 'delivered'].includes(
        status
      )
    ) {
      return res.status(400).json({
